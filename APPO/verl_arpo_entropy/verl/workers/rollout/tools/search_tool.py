...
class BingSearchTool(BaseTool):
    ...
    def __init__(
        ...
        proxy_base_url: str = "http://your-proxy.example.com"
    ):
        """
        ...
        Args:
            ...
            proxy_base_url: 公司内网代理网关地址，通过 X-Dst-Host/X-Dst-Scheme 转发到目标地址
        """
        # API configuration
        ...
        # 公司内网代理网关地址，通过 X-Dst-Host / X-Dst-Scheme 转发到目标地址
    ...
    def _make_request(self, query: str, timeout: int) -> requests.Response:
        """
        Send request to Brightdata API via the internal company proxy gateway.

        The original target is:
            POST https://api.brightdata.com/request

        Via the proxy gateway, the request becomes:
            POST http://your-proxy.example.com/request
            with headers X-Dst-Host: api.brightdata.com
                         X-Dst-Scheme: https
        ...
        # Prepare headers: forward original auth headers + proxy routing headers
        headers = {
            "Authorization": f"Bearer {self._api_key}",
            "Content-Type": "application/json",
            # Proxy gateway routing headers: tell the gateway where to forward
        ...
        # Send request through internal proxy gateway instead of directly to Brightdata
        proxy_url = f"{self._proxy_base_url}/request"
        print(f"[SearchTool] Sending request to proxy: {proxy_url}, zone={self._zone}")
        ...
        print(f"[SearchTool] Response status={response.status_code}, body_len={len(response.text)}, body_preview={repr(response.text[:200])}")
        return response
