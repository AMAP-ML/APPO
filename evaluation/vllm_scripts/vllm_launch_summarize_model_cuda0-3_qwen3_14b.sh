...
rm   /home/wangxucong.wxc/log/model1.log
rm   /home/wangxucong.wxc/log/model2.log

# Create log directory
mkdir -p logs

# Model path - all instances use the same model
MODEL_PATH="/data/Agent/models/Qwen3-14B"
...
CUDA_VISIBLE_DEVICES=0,1 nohup vllm serve $MODEL_PATH \
    ...
    --port 8004 > /home/wangxucong.wxc/log/model1.log 2>&1 &
INSTANCE1_PID=$!
...
CUDA_VISIBLE_DEVICES=2,3 nohup vllm serve $MODEL_PATH \
    ...
    --tensor_parallel_size 2 \
    --gpu-memory-utilization 0.75 \
    --port 8005 > /home/wangxucong.wxc/log/model2.log 2>&1 &
INSTANCE2_PID=$!
