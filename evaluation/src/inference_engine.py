...
class AsyncInference:
    ...
    async def process_sample_wrap(self, idx, question, answer, metadata=None):
        try:
    ...
    async def task_worker(self, task_queue, questions, answers, results, metadata_list=None):
        while not task_queue.empty():
            try:
                ...
                results[idx] = await self.process_sample_wrap(
                    idx, questions[idx], answers[idx], metadata=metadata
                )
            except Exception as e:
                import traceback
        ...
        for dataloader in self.data_loaders:
            ...
            for turn in self.args.turns:
                ...
                for _ in range(min(self.args.max_concurrent_requests, total_examples)):
                    workers.append(
                        asyncio.create_task(
                            self.task_worker(task_queue, questions, answers, results, metadata_list)
                        )
                    )
