...
class BingSearchTool(BaseTool):
        ...
        while True:
            ...
            try:
                ...
                response = requests.post(
                    proxy_url,
                    headers=headers_with_proxy,
                    json=payload,
                    timeout=timeout,
                )
                print(f"[SearchTool] Response status={response.status_code}, body_preview={repr(response.text[:200])}")
    ...
    async def execute(self, query: str, timeout: int = 120, **kwargs) -> str:
        """
