...
class BingSearchToolSDS(BingSearchTool):
    ...
    def extract_text_from_url(self, url, use_jina=False, jina_api_key=None, snippet: Optional[str] = None):
        ...
        try:
            ...
            if use_jina:
                ...
                # 通过 Brightdata 代理抓取网页，避免内网无法直连外网
                ...
                print(f"[FetchURL] Fetching via proxy: {url[:80]}")
                ...
                raw_content = proxy_response.text
                try:
        ...
        for i, doc_info in enumerate(relevant_info):
            ...
            if (
                ...
                if success:
                    ...
                    print(
                        f"Failed to extrace page content based on snippet! Use the ogirin page: {context[:150]}..."
                    )
    ...
    async def webpage_analysis_single(self, prompt) -> str:
        ...
        if result is None or not result.choices:
            print("webpage_analysis_single: generate() returned None or empty choices")
            return ""
