...
OUTPUT_PATH="/home/wangxucong.wxc/logs/log_${1}"
mkdir -p $OUTPUT_PATH
DATA_PATH="data"                       
TURNS="1 2 3 4 5"  #  
...
# Tool configurations
CONDA_PATH="/home/wangxucong.wxc/miniconda3"   # Conda installation path
CONDA_ENV="arpo"                                # Conda environment name
PYTHON_MAX_CONCURRENT=32                        # Max concurrent Python executor
BING_API_KEY="${BING_API_KEY:-}"                 # Bing Search API key
BING_ZONE="${BING_ZONE:-your_zone}"              # Bing search zone
SEARCH_MAX_RESULTS=10                            # Max number of search results
SEARCH_RESULT_LENGTH=1000                        # Max length per search result
BING_REQUESTS_PER_SECOND=4.0                    # Max Bing search requests per second
...
SUMM_MODEL_PATH="/data/Agent/models/Qwen3-14B"
...
# Execute command
eval $CMD >> /home/wangxucong.wxc/logs/log_${1}/infer.log 2>&1
